...
handleLibs <- function(action) {
  ...
  if (os == "Windows") {
      ...
      micro_path = paste0(Sys.getenv("$AZ_BATCH_NODE_ROOT_DIR"), "/NicheMapR/libs/linux/MICROCLIMATE.so")
      ecto_path =  paste0(Sys.getenv("$AZ_BATCH_NODE_ROOT_DIR"), "/NicheMapR/libs/linux/ECTOTHERM.so")
  } else if (os == "Darwin") {
